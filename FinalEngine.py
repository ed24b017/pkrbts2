'''
Trainer engine for self play

I'm not very sure if I'm instantiating the trainer class in hte correct place
I need it to persist actross hands and i need to call trainer.plot_results at the end to visualize convergence and save strategy data.

I'm not sure if the utility function is correct. Too many nested ifs and loops. I wanna sleep;.
'''
...
class Trainer():
    def __init__(self):
        ...
        self.current_reach_probab = 1

    def utility(self, infoset):
        current_node = infoset[0]
        nodes_to_terminal = 5 - infoset[0]
    ...
    def board_reveal_card(self, infoset):
        current_node = infoset[0]
        """node ={
            0: 'auction',
            1: 'pre-flop',
        ...
        if current_node == 2:
            board_need = 3
        if current_node == 3 or current_node == 4:
            board_need = 1
        if current_node == 0 or current_node == 5:
            return infoset
        cards = infoset[2]
        my_hand = list(cards[0])
        board = list(cards[1])  # Make mutable copy
        opp_revealed = list(cards[2])
        dead = set(my_hand + board + opp_revealed)
        deck = [eval7.Card(r + s) for r in '23456789TJQKA' for s in 'shcd' if (r + s) not in dead]
        draw = random.sample(deck, board_need)
        board.extend(draw)
        infoset = [infoset[0], infoset[1],(my_hand, board, opp_revealed), infoset[3], infoset[4], infoset[5], infoset[6],infoset[7]]
        return infoset

    def playoff(self, infoset, choice):
        current_node = infoset[0]
        nodes_to_terminal = 5 - infoset[0]
        bucket = infoset[1]
        cards = infoset[2]
    ...
    def get_new_legal_actions(self, infoset):
        ...
        my_balance = infoset[6]
        opp_balance = infoset[7]
        cost_to_call = if_win + if_loss  # opp_bet - my_bet
        if cost_to_call == 0:
            available.append(2)  # check
        else:
            available.append(1)  # call
                # raises are available if both players have chips left
        if my_balance-20 > 0 and opp_balance-20 > 0:
            available.extend([3, 4, 5])  # raise_small, raise_big, all_in
        return available
    ...
    def simulate_opponent_action(self, infoset):
        ...
        if_loss = - int(infoset[3]) # - if_win = - opp_bet
        my_balance = int(infoset[7])
        opp_balance = int(infoset[6])
        bid = 0
        ...
        if action == 1:  # call
            bid = 20
        if action == 3:  # raise_small
            bid = min(opp_balance, my_balance, 0.5 * opp_balance)
        if action == 4:  # raise_big
            bid = min(opp_balance, my_balance, 0.75 * opp_balance)
        if action == 5:  # all_in
            bid = min(opp_balance, my_balance)
        
        return -if_loss-bid,-if_win+bid, my_balance - bid, opp_balance - bid
        # Recheck this return for logical consistency!!! I'm very sleepy
    ...
    def mc_evaluate(self, infoset): 
        ...
        if wins + 0.5 * ties > loss: return True
        else: return False            
    
    def regret_matching(self, infoset):
        # compute strategy from regret
        street = infoset[0]
        bucket = infoset[1]
        positive_regret = np.maximum(self.regret[street][bucket], 0)
        if np.sum(positive_regret) > 0:
    ...
    def mccfr(self, infoset, reach_probab):
        ...
        self.current_reach_probab = reach_probab # bot reach probability
        node_utilities = self.utility(infoset)
        # compute regrets and update regret table
        regret = np.zeros(NUM_ACTIONS)
        street = infoset[0]
        bucket = infoset[1]
    ...
    def plot_results(self):
        ...
        plt.plot(self.delta_regret)
        plt.yscale('log')
        plt.xlabel('Iteration')
        plt.ylabel('Delta Regret (log scale)')

        plt.subplot(1, 2, 2)
        plt.title('Strategy Convergence (Entropy)')
        plt.plot(self.strategy_convergence)
        plt.xlabel('Iteration')
...
class BotProcess:
    ...
    def stop(self):
        ...
        with open(os.path.join(GAME_LOG_FOLDER, self.name + '.plog'), 'wb') as log_file:
            ...
            while not self.bytes_queue.empty():
                output = self.bytes_queue.get_nowait()
                try:
    ...
    def query(self, state, player_message, game_log, round_num):
        ...
        try:
            ...
            self.socketfile.write(message) # type: ignore
            self.socketfile.flush() # type: ignore
            clause = self.socketfile.readline().strip() # type: ignore
...
class PokerMatch():
    ...
    def run(self):
        ...
        if any(p.socketfile is None for p in players):
            print("One or more bots failed to connect. Aborting.")
            for p in players:
                p.stop()
            return
